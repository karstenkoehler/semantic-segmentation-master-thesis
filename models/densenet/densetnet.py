...
def DenseNet(dense_block_layers=None, growth_rate=16, initial_nb_filters=48, compression=1.0, bottleneck=True,
             dropout=0.2, model_name_suffix="", input_size=(256, 256, 3)):
    ...
    for i, block_size in enumerate(dense_block_layers):
        x, nb_filters = _dense_block(x, block_size, nb_filters, growth_rate, dropout, bottleneck)
...
def _convolution_block(x, nb_filters, dropout_rate=None, bottleneck=False):
    if bottleneck:
        x = BatchNormalization(gamma_regularizer=_l2reg(), beta_regularizer=_l2reg())(x)
        x = Activation("relu")(x)
        x = Conv2D(nb_filters * 4, (1, 1), padding="same", use_bias=False, kernel_regularizer=_l2reg())(x)
...
def _dense_block(x, nb_layers, nb_filters, growth_rate, dropout_rate=None, bottleneck=False):
    for _ in range(nb_layers):
        block = _convolution_block(x, growth_rate, dropout_rate, bottleneck)
