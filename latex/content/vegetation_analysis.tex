...
The goal of the vegetation analysis using SVIs is to estimate the vegetation density in given areas. In general, there are several factors which make it difficult to evaluate this goal. For example, the real vegetation levels not known to the authors and can only be estimated based on the image data. Additionally, various cameras and sensors were used to record the data (see \WIP{section~\ref{sec:dataset_analysis}}).
...
\begin{itemize}
    ...
    \item is it necessary to have the SVIs available as docker images?
\end{itemize}
}
