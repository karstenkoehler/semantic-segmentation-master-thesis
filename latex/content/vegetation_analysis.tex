...
\WIP{
evaluate SVI on given dataset. some example image with mostly agriculture and forest selected. assumption: forest does not necessarily have more vegetation. depends on type of trees and crops.
...
%%% RVI
\ref{fig:vegetation_rvi_examples} has results from RVI. challenge was to find proper visualization, because not limited. over entire dataset, values in range from 0 up to above 100 were found. not taking outliers into account. mapped values from 0-20 to 8-bit B/W image. Dark spots is no vegetation, lighter spots is increasing vegetation.

Figure shows, brightest spots are green forests. Forest without leaves or needles are much darker. this is plausible.

For agricultural field the separation is not as easy. RVI shows different shades of gray for those, so density of vegetation differs. You can see that index value correlates with saturation of green color in the image.

%%% NDVI, EVI
Figure \ref{fig:vegetation_ndvi_examples} shows results of NDVI and EVI. Both were mapped from range $-1$ to $1$, bright meaning more vegetation. There are big differences in the vegetation predictions.

for example, first column. NDVI predicts very dense vegetation except lane tracks. EVI is quite the opposite, lane tracks are brighter than the rest (i. e. more vegetation on tracks).

Regarding other images, EVI highlights forest pretty well. Trees with leaves or needles show bright spots, whereas ones without leaves or needles are rather dark. This differentiation is not as clear in NDVI.

However, NDVI is more sensitive on agricultural fields. second column highlights one part of the field significantly, where RGB only shows slight difference.
