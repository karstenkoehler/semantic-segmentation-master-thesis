...
SPP usually applied instead of last conv layer before FC layers. maintains spatial information by having spatial bins in proportion to image size. multiple levels are used (thus pyramid). level 0 is doing global max pooling, resulting in 1 value for every feature map. level 1 has 4 bins (2x2 structure) and level 2 has 16 bins (4x4 structure). all they do is max pooling inside every bin. this leads to fixed-length outputs for arbitrary input sizes. Figure~\ref{fig:spp_pooling} shows SPP layer in action.
}
...
This architecture had performance issues, training/test took very long. first improved in \cite{f-rcnn14} by first computing feature maps. Then use RoI pooling (special kind of max pooling) to identify RoIs in feature map. This way, a lot of time is saved because convnet is only used once.
