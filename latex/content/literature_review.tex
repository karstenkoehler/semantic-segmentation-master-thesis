...
\WIP{
\begin{itemize}
    % TODO: explain hyperparameters, grid search
    \item hyperparameter: LR, epochs, optimizer
...
\WIP{
\cite{wnet17}
Important difference to U-Nets is the use of depthwise separable conv layers introduced in \cite{depthwise_conv17}. depthwise separable convolution consist of depthwise convolution and pointwise convolution. Reason is to separate between spatial correlations and cross-channel correlations independently. Increases efficiency of the network with same number of parameters.

Authors apply postprocessing to output of W-Net to achieve better object delineation and smoothness. At first they use fully connected Conditional Random Field (CRF) \cite{crf17}. This allows for more confidence regarding boundaries of objects and removing small spurious regions. Second step is to reduce over-segmentation with hierarchical image segmentation. (over-segmentation is very common for unsupervised segmentation. better to have more categories and merge them later on) Importance of boundaries in over-segmented partitions is computed based on spectral clustering \cite{hierarchy_imgseg11}. This will merge some predicted segments to yield final segmentation results.
}

\subsection{Architecture Comparison}
\WIP{
overall, all three reference architectures that were introduced, share common architectural pattern. All have some kind of contracting path followed by expansive path. In general, it is very common to have decrease spatial size of feature maps while increasing number of feature maps (contracting path) for CNNs. it is natural to just invert that part if the result should be of same resolution as original image.

Hard to compare the actual performance of three architectures, since all were trained on different datasets with different metrics. Still, we look at some numbers and take into account the specific task with its difficulty. Also, figure~\ref{fig:segmentation_examples} show some results for segmentation, to get a feeling for what was achieved.

U-Net was trained and evaluated for ISBI cell tracking challenge described in \cite{isbi_challenge}. Two different datasets in the challenge, both showing cells scanned with different microscopy techniques. achieved an average IoU of $92\%$ for PhC-U373 and $77.5\%$ for DIC-HeLa respectively \cite{unet15}. One examples for DIC-HeLa dataset is shown in figure~\ref{fig:exseg_unet}. This is a binary segmentation with two classes (cell, not-cell), meaning the prediction tells if pixel belongs to a cell or not.

DenseNet for segmentation was trained and evaluated with CamVid dataset \cite{camvid_challenge}. This is database of urban scene videos fully labelled with 32 different classes. Classes include moving objects (like pedestrian, bicyclist, car), road (driveable road, lane marking, etc), ceiling types (sky, tunnel) and fixed objects (buildings, fences, etc). DenseNet achieved mean IoU of $66.9\%$, which is less than U-Net had for their challenge. On the other hand, the challenge is much more sophisticated (see figure~\ref{fig:exseg_densenet}). As another metric, authors present $91.5\%$ pixel-wise accuracy. This shows, predictions in general are of high quality, only outlining of the objects could be improved.

W-Net trained on PASCAL VOC 2012 dataset and evaluated with Berkeley Segmentation Database (BSDS500). Both datasets contain hundreds of images with labelled segmentations. But since W-Net is unsupervised labels are not used during training, but only for evaluation. Metrics are a bit different in this case, because there are multiple ground-truth segmentations for each image. Two metrics of interest: \emph{probabilistic rand index} (PRI) is a form of pixel-wise accuracy. Other metric is \emph{segmentation covering} (SC), which is extended IoU. In detail explanation of metrics can be found in \cite{hierarchy_imgseg11}.

W-Net without postprocessing scores for BSDS500 dataset values of up to $62\%$ SC and $84$ PRI. If compared to mean IoU and pixel-wise accuracy of DenseNet, the performance seems to be worse. But taking into account, that the W-Net does not receive any labelling information during training, the results are still valuable (as figure~\ref{fig:exseg_wnet} shows).
