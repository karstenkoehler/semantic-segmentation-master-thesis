...
% TODO: raise ideas how the methods can be used throughout the thesis
}
...
\WIP{
\cite{wnet17}
U-net and dense-net both use supervised learning. Also approaches with unsupervised learning exist, one called W-Net. Extension to U-Net architecture, basically two U-Nets concatenated, thus resulting in W shape. First FCN encodes image to k-way soft segmentation. second FCN goes the other way: from segmentation back to reconstructed image. Goal is to minimize reconstruction error and normalized cut loss function of encoding layer. Very similar to autoencoder architectures (provide paper for that).

Network architecture illustrated in figure~\ref{fig:wnet_architecture}. First part is $U_{Enc}$, encodes image to segmentation. Very similar to U-Net, contracting path in the beginning, expansive path after that. input of layers with same spatial extents is bypassed to expansive path to enable precise feature localization. Since $U_{Enc}$ does segmentation, output of it is one-hot encoded segmentation map. Last layer is 1x1 conv layer with softmax activation. The number of classes is determined by number of feature maps in output layer (denoted by $K$ in the figure). $U_{Dec}$ is very similar. But it reads input with $K$ feature maps and outputs 3 feature maps representing RGB values in original image.
...
\WIP{
\begin{itemize}
    \item Interesting for architecture comparison: \cite{imseg_architecures}
