...
\WIP{
DNNs are very flexible and allow to solve broad range of tasks. however, flexibility also comes with disadvantages. they allow for countless configuration options that all affect the final outcome. the options are called hyperparameters \cite[pp.~270ff]{praxiseinstieg_ml17}.

in general, hyperparameter is option of learning algorithm. it is not learned during training, but rather set beforehand. hyperparams include all kinds of things, like loss and activation function. also architecture of network can be seen as hyperparameter. option are e.g. number of layers, number of neurons per layer, etc.

a major challenge is to find optimal hyperparams for the task. since training is already very time consuming, it is not feasible to explore all permutations of hyperparams. there are two major strategies for optimizing hyperparameters of existing model: grid search and random search \cite[pp.~73ff]{praxiseinstieg_ml17}.

for grid search, some options for all hyperparameters are specified. grid search then executes all permutation of given hyperparams and reports results for each one. this strategy is well suited if the number of combinations is rather low.

otherwise, random search can be consulted. works the same way, but only picks some random samples from the permutations. speeds up the search significantly, but results are less informative. Usually, random search still allows to recognize some optimization patterns in hyperparams, that can be further explored.
}
