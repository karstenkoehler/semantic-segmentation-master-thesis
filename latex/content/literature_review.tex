...
\begin{figure}
    ...
    \hfill
    \begin{subfigure}{\CategoricalAccuracyImageWidth}
        \includegraphics{images/categorical_accuracy_label}
        \caption{Ground Truth}
        \label{fig:ca_truth}
    \end{subfigure}
    \hfill
    \begin{subfigure}{\CategoricalAccuracyImageWidth}
        \includegraphics{images/categorical_accuracy_prediction}
...
\WIP{
\cite{tf_whitepaper15}
categorical accuracy measures the ratio of correct label predictions compared to total number of predictions. predictions are considered as correct if class with highest score matches class in label. for image segmentation, categorical accuracy can be seen as percentage of pixels in the image that were predicted correctly (see equation~\ref{eq:categorical_accuracy}). In general, yields good impression for prediction performance.
...
Drawbacks when it comes to imbalanced class distributions in images. for example, if there is dominant class in most images (like background class), predictions can always predict dominant class and categorical accuracy will be very good. Figure~\ref{fig:categorical_accuracy} shows example for this. Categorical accuracy for this case will evaluate to around $95\%$, but predictions are worthless. Consequence is that high categorical accuracy does not imply good segmentation predictions.

another metric which is not prone to this is intersection over union (IoU) like in~\cite{pascal_voc15}. defines ratio between area of overlap in predictions to union of prediction and ground truth (see equation~\ref{eq:mean_iou}). IoU computed for each class separately and then averaged over all classes. Calculating that for example in figure~\ref{fig:categorical_accuracy} above, mean IoU is around $47\%$ (assuming $95\%$ for background and $0\%$ for ships). Mean IoU is much better metric for success, because it returns more stable results.
...
\begin{itemize}
    \item metrics: Mean IoU, Categorical Accuracy
    \item losses: MSE, binary crossentropy, categorical crossentropy
\end{itemize}
}
