...
\WIP{
\cite[p.~214f]{dlma14}
deep learning emerged as new field of machine learning. using many layers with non-linear information processing.
...
\cite[p.~4]{dl_overview14}
standard neural network consists of simple computational units called neurons.

\cite[pp.~125f]{nn_intro96}
traditional neural network is a function to map input vector to specific output vector. consists of neurons (or perceptrons), which calculates weighted sum of its inputs, applies activation function and outputs single scalar value. for NN, neurons form directed graph where edges define the weights between output and input of next neuron. training process is essentially updating the weights to achieve better matching prediction.

\cite[pp.~125f]{nn_intro96}
from topological point of view neurons are usually grouped into layers. first layer that receives input is called input layer. last layer to make prediction is called output layer. in between is arbitrary amount of hidden layers. architectures tend to increase number of hidden layers, thus creating the term of deep learning. topology of network is often referred to as architecture.
