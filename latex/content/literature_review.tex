...
\WIP{
another metric which is not prone to this is intersection over union (IoU) like in~\cite{pascal_voc15}. defines ratio between area of overlap in predictions to union of prediction and ground truth (see equation~\ref{eq:mean_iou}). IoU computed for each class separately and then averaged over all classes. Calculating that for example in figure~\ref{fig:categorical_accuracy_drawbacks} above, mean IoU is around $47\%$ (assuming $95\%$ for background and $0\%$ for ships). Mean IoU is much better metric for success, because it returns more stable results.
