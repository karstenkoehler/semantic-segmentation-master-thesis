...
\begin{figure}
    ...
    {\textbf{Metrics of U-Net Configurations}. Calculated per epoch during training from validation set. (\subref{fig:metrics_unet_loss})~Loss. (\subref{fig:metrics_unet_catacc})~Categorical Accuracy. (\subref{fig:metrics_unet_iou})~Mean IoU. }
...
\begin{figure}[h]
    ...
    {Calculated per epoch during training from validation set. (\subref{fig:metrics_densenet_loss})~Loss. (\subref{fig:metrics_densenet_catacc})~Categorical Accuracy. (\subref{fig:metrics_densenet_iou})~Mean IoU.}
...
\begin{figure}[h]
    ...
    {\textbf{Large Scale Predictions of \texttt{FC-DenseNet-67D} and \texttt{U-Net-23D}}. Images cover around $580\times 580~\text{meters}$.
