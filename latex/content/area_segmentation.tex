...
\WIP{
Lastly, look at W-Net architecture presented in section \ref{sec:wnet}. Implementation for U-Net is reused, but instead with separable conv layers and zero padding.

W-Net is trained unsupervised, meaning the labels of the data set are not used. instead, original image is used as both input and label. the model has to figure out the classes by itself.

Image size for this set to $256\times 256$ pixels. For training we employ Adam optimizer with learning rate of $0.001$. Loss is calculated by a categorical crossentropy function.

Unlike original architecture, we don't use two-part training function. instead, results of encoder are argmaxed and fed into decoder. that way, only six distinct values ever reach decoder. So encoder has to provide meaningful classes, in order for the decoder to produce useful results.
}

\subsubsection{Metrics}
\WIP{
metrics for W-Net are different from the ones presented for U-Net and FC-DenseNet. Since no labels used in training, it is not possible to calculate categorical accuracy and mean IoU. this is because class predictions of encoder does not necessarily match with predefined classes in labels.

only metric presented in figure \ref{fig:wnet_train_metrics} is categorical crossentropy loss function. since models do not show any improvements during training we only train a single epoch. graphs plots loss per batch in training data. shows difference between original image and reconstructed image from W-Net. Notice scale on y-axis, barely any improvement over all batches.

all configurations show similar behavior in training. first few batches show adequate improvement. then, model starts to become worse again. at this point, no satisfying explanation for this behavior can be given.
}
...
\subsubsection{Images}
\WIP{
figure \ref{fig:wnet_prediction_images} shows visual results of W-Net configurations from test set. two images per configuration: one is predicted segmentation, other is reconstructed image. colors in predictions are not related to any of classes mentioned earlier, but rather are what model learned to differentiate.

clearly noticeable that most configurations did not learn broad variety of classes. mostly show one dominant class. especially for configurations with 20 internal classes this is surprising. they do not use condensed feature space to encode any information. thus, those configs were not able to produce valid reconstruction for image.

for 6 internal classes, reconstructions look better. especially \texttt{W-Net-46D-6} shows coarse texture of original image. this is also only config that makes notable use of 2 internal classes. Still, does not use entire feature space to encode information.

in summary, results of W-Net were very disappointing. Before results would have been applicable to solve real segmentation class, postprocessing would have to be performed on outputs. Due to poor segmentation results this was not done and W-Net in general will not be considered further throughout the thesis.
}
