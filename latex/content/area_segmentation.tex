...
\WIP{
this section discusses segmentation results in general.

W-Net performed very poorly. almost no differentiation between classes was learned. because of that, post-processing was not implemented. in total, W-Net approach was not very effective. for that reason, it is not persued any further.

U-Net and DenseNet results were generally way better. metrics not perfect, but images show that in general they are able to make useful predictions. up until now, only very small-sized areas have been explored. since real-world use case would be huge areas, we now look at larger scale images.

figure \ref{fig:segmentation_discussion} show images with edge length of around $300$ meters. show original image, label and predictions of FCDN-67D and U-Net-23D. Those configurations were chosen, because they performed best in analysis before (section \ref{sec:unet_experiments} and \ref{sec:densenet_experiments}). 

for Unet, relation to label is clearly visible. general outlines of classes look very similar. escpecially for classes buildings, forest, agriculture. this is expected, because those are the dominant classes in the data set. for those, there is lots of good training examples available. 

also water class is very accurate. in contrast to labels, even small islands in river were detected. however, some false positives in areas with dense and saturated grass. 

classes that did not perform well are urban green and traffic. traffic class was also labelled poorly in the data set. most roads were not even labelled as traffic area, so models can not learn that effectively. 

urban greens are also very hard to detect. since they are mostly parks and sport facilities, they are easy to confuse with agriculture or buildings. this is exactly what happened in the model's predictions. 

with regards to classes, densenet predictions can be explained similarly. however, there is one big difference in predictions: densenet has much finer resolution. this is escpecially true for building class. where U-Net predicted one big buildings segment, densenet distinguished between single houses. in that case, prediction is even better than the labels. 

However, densenet predictions in general seem very fragmented. transition between agriculture and forest not very sharp, more like fissured patchwork. To some extent, this represents true nature, but for making conclusions about landing fields it is not very helpful. 
...
% TODO: change positions of densenet and unet
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        \node[align=center] at (8.3, 0) {\tiny FC-DenseNet-67D};
        \node[align=center] at (12, 0) {\tiny U-Net-23D};
    \end{tikzpicture}
...
docker images are publicly available in docker hub in \texttt{imageseg/terrain-segmentation} repository. two tags in the repository, one for U-Net the other for DenseNet.
