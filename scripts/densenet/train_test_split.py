...
def add_test_set_column():
    stmt = "ALTER TABLE geom_tiles_densenet ADD test_set BOOLEAN DEFAULT FALSE;"
    with db.cursor() as cur:
        cur.execute(stmt)


def get_gids_intersecting_segment(segment_description):
    stmt = f"SELECT geom_tiles_densenet.gid FROM geom_segments, geom_tiles_densenet " \
           f"WHERE ST_Intersects(geom_segments.geom, geom_tiles_densenet.geom_label)" \
           f"AND geom_segments.segmentation_description='{segment_description}'" \
           f"ORDER BY geom_tiles_densenet.gid;"
    with db.cursor() as cur:
        cur.execute(stmt)
...
def set_test_data(gids):
    with db.cursor() as cur:
        for chunk in chunks(gids, 50):
            gid_list = ",".join(chunk)
            stmt = f"UPDATE geom_tiles_densenet SET test_set=TRUE WHERE gid IN ({gid_list})"
            cur.execute(stmt)

    train_dir = os.path.join("E:", "data", "densenet", "train")
    test_dir = os.path.join("E:", "data", "densenet", "test")
